...
function ModeGlyph({ mode }) {
  ...
  if (mode === 'tube') {
    return (
      <svg viewBox="0 0 64 64" className="w-7 h-7" aria-label="Underground" role="img">
  ...
  if (mode === 'bus') {
    return (
      <svg viewBox="0 0 64 64" className="w-7 h-7" aria-label="Bus" role="img">
  ...
  return (
    <svg viewBox="0 0 64 64" className="w-7 h-7" aria-label="Rail" role="img">
...
function getStepTokens(option) {
  switch (option.type) {
    ...
    case 'tube+train': return ['walk', 'tube', 'train', 'walk']
    case 'tube':      return ['walk', 'tube', 'walk']
    case 'overground': return ['walk', 'overground', 'bus', 'walk']
    case 'bus':       return ['walk', 'bus', 'walk']
    case 'train':
    default:          return ['walk', 'train', 'walk']
  }
}
    ...
    <div className="flex items-center gap-1.5 flex-wrap">
      {steps.map((step, i) => (
        <div key={`${step}-${i}`} className="inline-flex items-center">
          <span className="inline-flex items-center justify-center rounded-xl bg-slate-50 border border-slate-100 w-10 h-10">
            <ModeGlyph mode={step} />
          </span>
          {i < steps.length - 1 && <span className="mx-1 text-slate-200 text-xs">›</span>}
        </div>
      ))}
    </div>
  )
...
export default function RouteOptionCard({ option, isLast }) {
  ...
  const depsStr = departures.length > 0
    ? (nextThree.length > 0 ? null : 'No services')
  ...
  return (
    <div className={!isLast ? 'border-b border-slate-100' : ''}>
      <button
        ...
        className="w-full min-h-[88px] flex flex-col justify-center px-4 py-4 hover:bg-slate-50 active:bg-slate-100 transition-colors text-left"
      >
        <div className="flex items-start justify-between gap-3 w-full">
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              {nextThree.length > 0 ? (
                nextThree.map((m, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center bg-slate-900 text-white rounded-full px-2.5 py-0.5 text-xs font-bold"
                  >
                    {m} min
                  </span>
                ))
              ) : (
                depsStr && <span className="text-xs text-slate-400">{depsStr}</span>
              )}
              {total != null && (
                <span className="text-xs font-semibold text-amber-600 ml-auto">{total} min total</span>
              )}
            </div>
          ...
          <span className="text-slate-300 text-[9px] mt-1 shrink-0">{expanded ? '▲' : '▼'}</span>
        </div>
      </button>

      {expanded && (
        <div className="px-4 pb-4 pt-2 bg-slate-50/60">
          <p className="text-[11px] text-slate-400 mb-2 font-medium">Journey stages</p>
          <div className="space-y-2 mb-3">
            {stages.map((stage, idx) => (
              ...
              >
                ...
                <span className="text-xs font-bold text-slate-600 ml-2 shrink-0">
          ...
          {departures.length === 0 && serviceNote && (
            <p className="text-[11px] text-slate-400 italic mt-1">{serviceNote}</p>
          )}
          ...
          {departures.map((dep, i) => {
            ...
            return (
              <div
                key={dep.serviceID || i}
                className="flex items-center justify-between min-h-[44px] border-b border-slate-100 last:border-0"
              >
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-mono font-bold w-11 ${dep.isCancelled ? 'text-slate-300 line-through' : 'text-slate-900'}`}>
                    {dep.std}
                  </span>
                  {isDelayed && (
                    <span className="text-[10px] text-amber-500 bg-amber-50 rounded-md px-1.5 py-0.5 font-semibold">
                      {dep.etd}
                    </span>
                  )}
                  {dep.isCancelled && (
                    <span className="text-[10px] text-red-500 bg-red-50 rounded-md px-1.5 py-0.5 font-semibold">
                      Cancelled
                    </span>
                  )}
                </div>
                <span className={`text-sm font-bold ${dep.isCancelled ? 'text-slate-300' : isDelayed ? 'text-amber-500' : 'text-emerald-500'}`}>
