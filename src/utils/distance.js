export function calculateDistance(lat1, lon1, lat2, lon2) {
  const R = 6371
  const dLat = (lat2 - lat1) * Math.PI / 180
  const dLon = (lon2 - lon1) * Math.PI / 180
  const a =
    Math.sin(dLat / 2) * Math.sin(dLat / 2) +
    Math.cos(lat1 * Math.PI / 180) * Math.cos(lat2 * Math.PI / 180) *
    Math.sin(dLon / 2) * Math.sin(dLon / 2)
  return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a))
}

export function getNearestLocation(location, locations) {
  return locations.reduce((closest, item) => {
    const distance = calculateDistance(location.lat, location.lon, item.lat, item.lon)
    return !closest || distance < closest.distance ? { ...item, distance } : closest
  }, null)
}

export function walkingMinutes(fromLat, fromLon, toLat, toLon) {
  const distanceKm = calculateDistance(fromLat, fromLon, toLat, toLon)
  return Math.round((distanceKm * 1000) / 80)
}
